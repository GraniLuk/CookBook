...
```mermaid
sequenceDiagram
    participant P as Patelnia (nieprzywierająca)
    participant W1 as Wrap 1 (Spód)
    participant S1 as Sos Mięsny (336 g)
    participant M1 as Mozzarella (1/3)
    participant W2 as Wrap 2 (Środek)
    participant S2 as Sos Mięsny (336 g)
    participant M2 as Mozzarella (1/3)
    participant W3 as Wrap 3 (Środek)
    participant S3 as Sos Mięsny (336 g)
    participant M3 as Mozzarella (1/3)
    participant W4 as Wrap 4 (Góra)

    P ->> W1: Podgrzewanie na minimalnym płomieniu
    W1 ->> S1: Rozłożenie sosu (równo do brzegu, wypoziomować)
    S1 ->> M1: Posypanie startą mozzarellą
    M1 ->> W2: Ułożenie kolejnego wrapa
    W2 ->> S2: Rozłożenie sosu
    S2 ->> M2: Posypanie mozzarellą
    M2 ->> W3: Ułożenie kolejnego wrapa
    W3 ->> S3: Rozłożenie sosu
    S3 ->> M3: Posypanie mozzarellą
    M3 ->> W4: Ułożenie ostatniego wrapa (na sam środek)
```
