...
```mermaid
sequenceDiagram
    participant D as Dolna Warstwa (Ziemniaki + Bakłażan)
    participant M as Mięsne Nadzienie (Sos pomidorowy)
    participant B as Sos Beszamelowy
    participant P as Parmezan (Pozostałe 20 g)
    
    D -> M: Wyłożenie mięsa na upieczoną bazę
    M -> B: Nałożenie sosu beszamelowego (utworzenie równej powierzchni)
    B -> P: Posypanie całej góry pozostałym parmezanem
```
