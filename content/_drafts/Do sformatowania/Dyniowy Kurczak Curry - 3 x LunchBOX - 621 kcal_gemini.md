...
```mermaid
sequenceDiagram
    participant K as Kurczak (Pokrojony)
    participant C as Cebula (Kostka)
    participant D as Dynia (Puree)
    participant S as Składniki Sosu
    participant P as Patelnia

    K->>P: Smażenie na dużym płomieniu (7-8 min)
    P->>K: Odłożenie (Do miski, pod przykryciem)
    C->>P: Przesmażanie na resztkach tłuszczu
    P->>P: Deglazowanie wodą (100-150 ml)
    D->>P: Dodanie purée z dyni
    S->>P: Dodanie pomidorów, mleka kokosowego i przypraw
    P->>P: Gotowanie i redukcja na wolnym ogniu (5-6 min)
    P->>P: Doprawienie (Sól, Pieprz, Sok z cytryny)
    K->>P: Dodanie kurczaka z sokami
    P->>P: Mieszanie (2 min)
    P->>B: Gotowy Sos do Boxów
```
