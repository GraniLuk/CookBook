...
```mermaid
sequenceDiagram
    participant M as Mąka Tortowa (400g)
    participant Z as Ziemniaki (200g)
    participant A as Dodatki (Sól, Jajko)
    participant W as Gorąca Woda
    participant B as Masło (15g)
    
    M->>M: Wsypanie do dużej miski
    M->>A: Dodanie soli i jajka
    Z->>M: Dodanie przeciśniętych ziemniaków
    B->>W: Roztopienie masła w gorącej wodzie
    W->>M: Stopniowe dolewanie gorącej wody z masłem
    M->>M: Mieszanie, aż masa połączy się w duże grudki
    M->>M: Przejście na blat
```
