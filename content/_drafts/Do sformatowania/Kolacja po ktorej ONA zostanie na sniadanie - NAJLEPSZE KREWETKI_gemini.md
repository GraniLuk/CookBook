...
sequenceDiagram
    ...
    participant M as Makaron (Woda)
    participant K as Krewetki i Dodatki

    Note over P: Płomień mały, patelnia duża
    P->P: Dolać oliwę (letnią)
    P->P: Wrzucenie czosnku i chili
    Note over P: Aromatyzowanie oliwy (delikatny płomień)
    K->P: Wrzucenie krewetek (płomień można nieco zwiększyć)
    Note over P: Rozprowadzić, aby każda krewetka miała styczność z patelnią
    K->P: Obrócić krewetki na drugą stronę
    M->M: Wrzucenie makaronu do gotowania (START: 8 minut)
    K->P: Dodanie masła
    K->P: Dodanie szpinaku (ma zakryć patelnię) i suszonych pomidorów
    Note over P: Czekać aż liście szpinaku zwiotczeją
    K->P: Otwarcie wina i polanie (do połowy wysokości krewetek)
    Note over P: Średni płomień, czekać aż alkohol odparuje (zbieranie smaków ze ścianek)
    Note over P: Pozostały czas gotowania makaronu: 2 minuty
    M->P: Przerzucenie całego makaronu na patelnię (8 min minęło, makaron al dente)
    Note over P: Obrabianie całości na patelni (ok. 2 minuty)
    Note over P: Sos jest wchłaniany przez makaron spaghetti
    P->P: Wyłączenie płomienia
    P->M: Gotowe danie
```
