...
```mermaid
sequenceDiagram
    participant PCH as Papryczka Chili
    participant CZ as Czosnek
    participant IM as Imbir
    participant L as Limonka
    participant M as Moździerz

    PCH->>PCH: Oczyścić wewnątrz
    PCH->>PCH: Przekroić na pół
    PCH->>M: Dodać pierwszą połowę chili
    CZ->>CZ: Zgrubsza poszatkować
    CZ->>M: Dodać poszatkowany czosnek
    M->>M: Zmiażdżyć chili i czosnek (Nadanie koloru sosowi)
    IM->>IM: Kawałek sporego imbiru bardzo drobno przygotować/posiekać
    L->>L: Wyciśnięty sok z limonki
```
