...
```mermaid
sequenceDiagram
    participant B as Brokuł
    participant K as Kurczak
    participant S as Beszamele
    participant Z as Zapiekanka
    participant P as Piekarnik

    B->>+B: Ugotować brokuł
    B->>B: Blendować na krem
    K->>+K: Smażyć 600 g pierś
    K->>K: Dodać cheddar na wierzch
    S->>+S: Przygotować beszamele (masło + mąka + mleko + przyprawy)
    S->>S: Blendować brokuł i wmieszać do sosu
    Z->>+Z: Połączyć kalafior + kurczak + beszamele
    Z->>Z: Dodać cheddar, migdały, rozmaryn
    Z->>P: Piec 30 min pokrywą
    P->>P: Odkręcić, piec 15‑20 min bez pokrywki
```
