...
```mermaid
sequenceDiagram
    participant S as Spód Daktylowo-Owsiany
    participant M as Masa Sernikowa
    participant L as Lodówka
    participant O as Owoce Leśne
    participant G as Galaretka
    
    S -> M: Wylanie Masy Sernikowej na Spód
    M -> M: Delikatne wypoziomowanie
    M -> L: Chłodzenie I (30 min)
    L -> O: Masa stężała, wykładanie owoców (od największych do najmniejszych)
    O -> G: Zalanie wystudzoną Galaretką
    G -> L: Chłodzenie II (Aż galaretka stężeje)
    L -> R: Konsumpcja
```
