---
title: WOOWINA w sosie jakiej nie znasz - ROZPYWA sie w ustach. LunchBOX 2 warianty _ POLICZONA SZAMA_gemini
...
```mermaid
sequenceDiagram
    participant P as Przygotowanie Składników
    participant B as Brytfanka
    participant M as Mięso

    P->>P: Obieranie i krojenie cebuli w drobną kostkę
    P->>P: Krojenie suszonych pomidorów w kostkę
    B->>B: Rozgrzanie 20 ml oliwy
    B->>B: Wsypanie cebuli
    B->>B: Przesmażanie cebuli (do utraty surowości i miękkości)
    P->>M: Przygotowanie pręgi wołowej (krojenie w poprzek włókien, plastry 1-1.5 cm grubości)
    B->>B: Dodanie i rozpuszczenie 30 g masła klarowanego
    B->>M: Dodanie mięsa, równomierne rozłożenie na dnie
    B->>M: Przesmażanie z jednej strony (3-4 minuty)
    B->>M: Odwrócenie kawałków
    B->>M: Przesmażanie z drugiej strony (3-4 minuty)
    B->>B: Dodanie 1 łyżki pasty Yellow Curry
    B->>B: Dodanie 1 łyżeczki pasty Red Curry (Diabeł)
    B->>B: Dodanie suszonych pomidorów
    B->>B: Mieszanie składników
    B->>B: Dodanie suszonych ziół i gałązki rozmarynu
    B->>B: Zalanie wodą (ilość zakrywająca całe mięso)
    B->>B: Wymieszanie całości
    B->>B: Przykrycie i gotowanie na wolnym ogniu (2.5 do 3 godzin)
    M->>M: Stabilizacja temperatury i duszenie (mieszanie kilka razy)
```
