...
```mermaid
sequenceDiagram
    participant P as Piekarnik (180°C Termoobieg)
    participant M as Mięso (Kiełbaski)
    participant B as Bagietka Czosnkowa

    M->P: Wkładanie mięsa (0 min)
    Note over P: Pieczenie: 25-30 min
    P->P: Po 12 minutach: Zmiana ustawień
    P->P: Wyłączenie Termoobiegu
    P->P: Grzanie góra-dół (200°C)
    B->B: Nacięcie bagietki
    B->P: Wkładanie bagietki (ok. 15 min pieczenia mięsa)
    Note over P: Pieczenie bagietki: ok. 12 min (200°C)
    M->P: Wyciągnięcie mięsa (25-30 min)
    B->P: Wyciągnięcie bagietki
```
