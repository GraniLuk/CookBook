...
created: 2025-10-30
description: Ten odcinek prezentuje przepis na prosty, szybki, dwupoziomowy i dwukolorowy sernik wysokobiałkowy typu low carb, z dokładnym omówieniem składników, procesu mieszania i pieczenia.
category: Fitness
length: 732
---
...
```mermaid
sequenceDiagram
    participant P as Piekarnik
    participant S as Sernik
    P->>S: Rozgrzewanie do 180°C
    S->>P: Wstawienie do Piekarnika (Środek)
    P->>S: Pieczenie (około 55 minut)
    Note over P: Wyłączenie Piekarnika po 55 minutach
    S->>P: Chłodzenie Faza 1 (Wewnątrz, wyłączony piec, 30 minut)
    P->>P: Uchylenie drzwiczek (po 30 minutach)
    S->>P: Chłodzenie Faza 2 (Uchylone drzwiczki, 30 minut)
    S->>S: Wyjęcie i Całkowite Ostygnięcie
```
