...
```mermaid
sequenceDiagram
    participant K as Kalafior
    participant R as Robot Kuchenny/Tarka
    participant P as Patelnia
    participant M as Miska

    K->>R: Rozdrobnienie (pulsacyjnie)
    Note over R: Konsystencja surowej kaszy gryczanej (nie za drobno!)
    R->>P: Wysypanie na dużą patelnię
    P->>P: Podgrzewanie na średnim płomieniu (8-10 min)
    Note over P: Cel: Odparowanie wody i delikatne przypieczenie
    P->>M: Przeniesienie do miski (objętość zmniejszona)
```
