...
```mermaid
flowchart TD
    A[Rozpoczęcie] --> B{Ziemniaki}
    B --> C[Talarki, gotuj 15 min]
    C --> D[Odcedź]
    A --> E{Brokuł}
    E --> F[Ucinanie różyczek]
    F --> G[Gotuj na parze 5 min]
    G --> H[Al‑dente 10 min]
    A --> I{Jogurt‑feta sos}
    I --> J[Rozgnieść 50 g jogurt + 50 g feta]
    J --> K[Dodaj przyprawy]
    K --> L[Odstaw w lodówce]
    A --> M{Ser‑pieczarki‑parsley}
    M --> N[W misce: mozzarella, pieczarki, natka, majonez]
    A --> O[Schabik]
    O --> P[Wyjmij z lodówki, dopraw solą i pieprzem]
    O --> Q[Rozgrzej patelnię, smaż 3–4 min/stronę]
    Q --> R[Dodaj mozzarella, przykryj]
    R --> S[Podaj z chmurką i warzywami]
    S --> T[End]
```
