...
```mermaid
flowchart TD
    A[Rozpoczęcie] --> B{Gotowanie batata}
    B --> C[Bataty gotowe]
    C --> D{Krojenie cebuli}
    D --> E[Cebula pokrojona]
    E --> F{Smażenie cebuli}
    F --> G[Cebula gotowa]
    G --> H{Odciskanie batata}
    H --> I[Plasterki batata]
    I --> J{Mieszanie składników}
    J --> K[Łyżka mieszania]
    K --> L{Smażenie omletka}
    L --> M[Omlet smażony]
    M --> N{Obracanie omletka}
    N --> O[Omlet gotowy]
    O --> P{Serwowanie}
    P --> Q[Podawanie]
    Q --> Z[Zakończenie]
