...
```mermaid
sequenceDiagram
    participant P as Przygotowanie (10 min)
    participant M as Mięso i Warzywa na Blasze
    participant PK as Piekarnik (180°C Termoobieg)

    P -> M: Pokrojenie, przyprawienie mięsa (1/2 oleju)
    P -> M: Dodanie i przyprawienie warzyw (reszta oleju)
    M -> PK: Włożenie blachy (Warzywa na górze)
    PK -> PK: Pieczenie Etap 1 (20 minut)
    PK -> M: Wyjęcie i wymieszanie całości
    M -> PK: Włożenie blachy
    PK -> PK: Pieczenie Etap 2 (10 minut)
    PK -> M: Wyjęcie i studzenie
```
