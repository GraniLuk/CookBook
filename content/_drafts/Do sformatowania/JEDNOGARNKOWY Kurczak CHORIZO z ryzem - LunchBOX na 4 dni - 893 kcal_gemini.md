...
```mermaid
sequenceDiagram
    title Sekwencja Gotowania Dania Jednogarnkowego
    participant P as "Patelnia/Olej (20 ml)"
    participant CH as "Kiełbasa Chorizo"
    participant CE as "Cebula"
    participant PA as "Żółta Papryka"
    participant KU as "Kurczak (400 g)"
    participant R as "Ryż (400 g)"
    participant W as "Woda (800 ml)"
    participant D as "Dodatki/Doprawianie"

    P->>CH: Wrzucenie Chorizo
    Note right of CH: Obsmażanie, wytopienie tłuszczu. Olej staje się czerwony i pikantny.
    CH->>D: Wyjęcie Chorizo (wróci na koniec, aby zachować chrupkość)
    CE->>P: Wrzucenie Cebuli
    Note right of CE: Smażenie 2-3 minuty (do zeszklenia)
    PA->>P: Dodanie Papryki
    Note right of PA: Smażenie 1-2 minuty
    KU->>P: Dodanie Kurczaka
    Note right of KU: Szybkie ścięcie/przesmażenie na mocnym płomieniu (zamknięcie soczystości)
    R->>P: Wsypanie Ryżu
    Note right of R: Mieszanie, połączenie ryżu ze smakami na patelni.
    W->>P: Zalanie ciepłą/gorącą wodą (ok. 800 ml)
    P->>P: Przykrycie pokrywką
    P->>P: Rozpoczęcie odliczania (18 minut)

    loop Duszenie (18 minut)
        P->>P: Mieszanie (3-4 razy w trakcie)
        Note right of P: Po ok. 8 minutach (w połowie czasu) dodanie kolejnej porcji wody.
        D->>P: Doprawianie Solą (ewentualnie Pieprzem, ostrożnie ze względu na pikantność).
        D->>P: Dodanie Suszonych Pomidorów i Oliwek (ok. 5 minut przed końcem).
        P->>P: Dokładne wymieszanie i dalsze duszenie pod przykryciem.
    end
```
