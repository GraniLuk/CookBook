---
title: Tikka Masala KURCZAK z ryzem - LunchBOX na 3 dni_gemini
...
```mermaid
sequenceDiagram
    participant P as Przygotowanie Wstępne
    participant K as Kurczak (Patelnia Sucha)
    participant S as Sos (Patelnia Olej)
    participant B as Boxy

    P->P: Pokrojenie i marynowanie Kurczaka (Olej, Chili, Kumin, Cynamon)
    P->P: Krojenie Cebuli i Czosnku
    P->P: Miażdżenie Kolendry i Gorczycy w Moździerzu
    P->K: Rozgrzanie Patelni (Duży Ogień)
    K->K: Smażenie Kurczaka z obu stron
    K->S: Kurczak do miski (Pod Przykryciem)
    S->S: Redukcja Ognia
    S->S: Smażenie Cebuli (3 min)
    S->S: Dodanie Czosnku (1-2 min)
    S->S: Dodanie Kuminu i Zmiażdżonych Ziaren (Aktywacja)
    S->S: Dodanie Przecieru Pomidorowego
    S->S: Dodanie Kurkumy, Papryki i Soli
    S->S: Gotowanie i Redukcja Sosu (5-7 min)
    S->S: Minimalny Ogień
    S->S: Dodanie Śmietany (100 g)
    S->S: Dodanie Kurczaka i Płynów/Soków z Miski
    S->S: Zagotowanie i Mieszanie (2-3 min)
    S->B: Podział Ryżu Basmati na 3 Porcje
    S->B: Nałożenie Kurczaka Tikka Masala
    B->B: Dekoracja Kolendrą
```
