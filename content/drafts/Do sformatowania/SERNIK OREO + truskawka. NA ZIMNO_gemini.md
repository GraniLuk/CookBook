...
```mermaid
sequenceDiagram
    participant D as Daktyle suszone (150g)
    participant W as Gorąca Woda (150 ml)
    participant Z as Żelatyna (15g)
    participant WZ as Gorąca Woda (150 ml)
    
    D->>W: Zalanie
    Note over D,W: Zaczynamy od namoczenia daktyli
    
    Z->>WZ: Rozpuszczenie
    Note over Z,WZ: Przygotowanie roztworu żelatyny
```