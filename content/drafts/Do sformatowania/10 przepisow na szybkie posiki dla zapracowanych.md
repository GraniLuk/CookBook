...
sequenceDiagram
    ...
    participant M as Montaż
    participant S as Sos

    P->>P: Pokrój Awokado (metoda: krojenie, potem obieranie)
    P->>P: Pokrój Pomarańczę (ćwiartki, usuń skórę)
    P->>P: Pokrój Cebulę Dymkę (w piórka)
    M->>M: Wyłóż bazę (Rukola/Mix sałat)
    M->>M: Ułóż Pomarańcze, Awokado, Cebulę
    M->>M: Umieść Burratę na środku
    S->>S: Wymieszaj Oliwa, Ocet Balsamiczny, Miód, Pieprz, Sól
    M->>M: Polej Sałatkę Sosem
    Note right of M: Sałatka gotowa
```
