...
Uwaga: Rzeczywiste wartości mogą się różnić (wilgotność, marka produktów, utrata masy w pieczeniu). Podane liczby przy 1 porcji odpowiadają dostarczonym danym referencyjnym – sumy składników są przybliżone (różnica energetyczna wynika z zaokrągleń i uśrednień etykiet).

## Wersja (bardziej) Low FODMAP – wskazówki (opcjonalnie)

1. Zastąp mąkę pszenną mieszanką: 130 g mąki ryżowej + 80 g skrobi kukurydzianej + 50 g skrobi ziemniaczanej + 0.5 łyżeczki gumy ksantanowej.  
2. Krem & batony: użyj 70% czekolady (limit ok. 15–20 g na porcję) + domowy krem z orzechów laskowych (≤15 g orzechów/porcja).  
3. Cukier częściowo zastąp erytrolem (np. 60–70%).  
4. Kontrola porcji: nie więcej niż 1 mały kawałek przy adaptacji.  
5. Tekstura może być bardziej krucha – można dodać 1–2 łyżki mleka bezlaktozowego dla balansu wilgotności.

---

Jeśli chcesz, mogę od razu przygotować alternatywną wersję front matter dla adaptacji low FODMAP (oddzielny plik lub wariant). Daj znać, czy coś doprecyzować albo zmodyfikować. 

Aby dodać obraz: umieść plik (np. `brownie_kinder_bueno.avif`) w `static/images/recipe-headers/` i upewnij się, że nazwa zgadza się z `recipe_image`.---
draft: false
title: Brownie Kinder Bueno
author: Bartłomiej Włodarczyk
recipe_image: images/recipe-headers/brownie_kinder_bueno.avif
